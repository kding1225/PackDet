...
class PACKDETLossComputation(object):
    ...
    def get_sample_region(self, gt, strides, num_points_per, gt_xs, gt_ys, radius=1.0):
        ...
        beg = 0
        for l, n_p in enumerate(num_points_per):
            end = beg + n_p
            stride = strides[l] * radius
    ...
    def prepare_targets(self, points, levels, targets, shapes):
        ...
        for l, points_per_level in zip(levels, points):
        ...
        labels, reg_targets = self.compute_targets_for_locations(
            points_all_level, levels, targets, expanded_object_sizes_of_interest
        )
        ...
        for l, level in enumerate(levels):
            labels_level_first.append(
                torch.cat([labels_per_im[l] for labels_per_im in labels], dim=0)
            )

            reg_targets_per_level = torch.cat([
                reg_targets_per_im[l]
    ...
    def compute_targets_for_locations(self, locations, levels, targets, object_sizes_of_interest):
        labels = []
        reg_targets = []
        ...
        for im_i in range(len(targets)):
            ...
            if self.center_sampling_radius > 0:
                is_in_boxes = self.get_sample_region(
                    bboxes,
                    [self.fpn_strides[l] for l in levels],
    ...
    def __call__(self, montage_info, box_cls, box_regression, centerness, targets):
        ...
        shapes = [x.shape[-2:] for x in box_cls]
        locations = montage_info["locations_map"]
        levels = montage_info["levels"]
        labels, reg_targets = self.prepare_targets(locations, levels, targets, shapes)  # level first
...
def make_packdet_loss_evaluator(cfg):
