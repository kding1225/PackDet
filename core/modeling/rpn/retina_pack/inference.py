...
from core.structures.boxlist_ops import boxlist_nms
...
def make_retinanet_postprocessor(config, rpn_box_coder, is_train):
    pre_nms_thresh = config.MODEL.RETINAPACK.INFERENCE_TH
    pre_nms_top_n = config.MODEL.RETINAPACK.PRE_NMS_TOP_N
    nms_thresh = config.MODEL.RETINAPACK.NMS_TH
    ...
    box_selector = RetinaNetPostProcessor(
        ...
        min_size=min_size,
        num_classes=config.MODEL.RETINAPACK.NUM_CLASSES-1,
